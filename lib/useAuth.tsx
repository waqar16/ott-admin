...
import { User, login as apiLogin, signup as apiSignup, me as apiMe, refreshToken as apiRefreshToken, logout as apiLogout } from './authApi';
...
interface AuthContextValue {
  ...
  isLoggedIn: boolean;
  loading: boolean;
  login: (email: string, password: string, token2fa?: string) => Promise<{ twoFactorRequired?: boolean ,role?:string}>;
  signup: (payload: { email: string; password: string; name: string }) => Promise<{ status?: number, access_token?:string, refresh_token?:string }>;
  logout: () => Promise<void>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
   ...
   */
  const fetchUser = useCallback(async (accessToken: string, retry = true): Promise<boolean> => {
    try {
   ...
   */
  const login = useCallback(async (email: string, password: string, token2fa?: string) => {
    setLoading(true);
    try {
  ...
  const signup = useCallback(async (payload: { email: string; password: string; name: string }) => {
    ...
    try {
      ...
      return { access_token: response.access_token,
        refresh_token: response.refresh_token,
      status:response.status}      // Fetch user profile

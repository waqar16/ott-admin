    ...
    <div className="mb-10">
      <div className="flex items-center justify-between mb-3 px-2">
        ...
        <div className="flex gap-2">
          <button onClick={() => scrollBy(-1)} aria-label="Scroll left" className="px-2 py-1 rounded bg-white/10 text-white hover:bg-white/20">◂</button>
          <button onClick={() => scrollBy(1)} aria-label="Scroll right" className="px-2 py-1 rounded bg-white/10 text-white hover:bg-white/20">▸</button>
        </div>
      </div>
      <div ref={scrollRef} className="flex gap-3 overflow-x-auto scrollbar-hide px-2 pb-2">
        {items.map((t) => (
          <Link key={t.id} href={`/title/${t.id}`} className="group relative flex-shrink-0 w-40 md:w-48">
            ...
            <div className="absolute bottom-2 left-2 right-2 flex flex-wrap gap-1 opacity-0 group-hover:opacity-100 transition">
              {(badgeExtractor ? badgeExtractor(t) : defaultBadges(t)).slice(0,3).map((b) => (
                <span key={b} className="px-2 py-0.5 text-[10px] rounded bg-black/70 text-white">{b}</span>
              ))}
            </div>
            {t.visibleWithoutSignup && (
              <span className="absolute top-2 left-2 bg-green-600 text-[10px] font-bold px-2 py-1 rounded">FREE</span>
            )}
          </Link>
        ))}
        {items.length === 0 && (
          <div className="text-gray-400 text-sm px-2">No items</div>
        )}
      </div>
    </div>
  );

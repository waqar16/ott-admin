...
const ShakaPlayer = dynamic(() => import('../ShakaPlayer/ShakaPlayer'), { ssr: false })

const VRAframePlayer = dynamic(() => import('../VrAframePlayer/VRAframePlayer'), { ssr: false })

// sonner MUST also be client-only
const toastPromise = dynamic(() => import('sonner').then((m) => m.toast), { ssr: false })
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 ">
      <div className="bg-neutral-800 rounded-lg   w-full my-8">
        ...
        <div className=" p-6 max-h-[calc(100vh-100px)] overflow-y-auto minimal-scrollbar space-y-6">
          {/* Basic Info */}
          <div>
            <h3 className="text-xl font-bold mb-4 text-white">{detailContent.title}</h3>
            ...
            {detailContent.content_metadata && (
              <div className="  rounded-lg space-y-4 mt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <MetaRow label="Directors" value={detailContent.content_metadata.directors} />
                  <MetaRow label="Producers" value={detailContent.content_metadata.producers} />
                  <MetaRow label="Cast" value={detailContent.content_metadata.cast} />
                  <MetaRow label="Genres" value={detailContent.content_metadata.genres} />
                  <MetaRow label="Awards" value={detailContent.content_metadata.awards} />
                  <MetaRow
                    label="Subtitles"
                  ...
                  />
                  <MetaRow label="Age Rating" value={detailContent.content_metadata.age_rating} />
                  <MetaRow label="Language" value={detailContent.content_metadata.language} />
                  <MetaRow
                    label="Production Company"
                    value={detailContent.content_metadata.production_company}
                  />
                  <MetaRow label="Country" value={detailContent.content_metadata.country} />
                </div>
              </div>
            )}
          </div>
          ...
          {/* Images */}
          {
            <div className="  grid grid-cols-7 gap-4 h-[70vh]">
              {detailContent.poster_url ? (
                ...
                <div className="h-full col-span-2 w-full flex flex-col items-center justify-center bg-neutral-950 rounded-lg ">
              ...
              {detailContent.banner_url ? (
                ...
                <div className="h-full col-span-5 w-full flex flex-col items-center justify-center bg-neutral-950 rounded-lg">
      ...
      {publishOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md">
          <div className="w-full max-w-md bg-neutral-900 border border-neutral-800 rounded-xl p-6 shadow-xl">
            <h2 className="text-lg font-semibold text-white mb-2">Confirm Publish Action</h2>

            <p className="text-sm text-gray-400 mb-4">
              Type <span className="text-white font-medium">"{detailContent.title}"</span> to
              continue.
            </p>
...
const MetaRow = ({ label, value }: { label: string; value?: string | string[] }) => {
    ...
    <div>
      ...
      {Array.isArray(value) ? (
        <div className="flex flex-wrap gap-2">
          {value.map((v) => (
            <span key={v} className="px-2 py-1 bg-gray-700 rounded text-xs text-white">
              {v}
            </span>
          ))}
        </div>

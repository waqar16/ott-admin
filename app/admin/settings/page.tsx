...
export default function AdminSettingsPage() {
  ...
  async function handleSave() {
    setLoading(true)
    try {
      const { site_name, support_email, primary_color, secondary_color, currency_default } =
        platformSettings
...
function DangerButton({ text }: { text: string }) {
  return (
    <button className="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700">{text}</button>
  )
}
    ...
    <div className="bg-neutral-950 border border-blue-100 rounded-xl p-6 shadow-sm">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">End your current admin session securely</p>
        </div>

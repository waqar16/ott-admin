      ...
      try {
        ...
        if (data.status === 'COMPLETE' || data.status === 'FAILED') {
          if (data.status === 'COMPLETE') {
            ...
            toast.error('Episode transcoding failed')
          }
    ...
    seriesList.forEach((series) => {
      series.children?.forEach((season: any) => {
        season.children?.forEach((ep: any) => {
          const ingestStatus = ep.ingest_status?.toLowerCase()
          ...
          if (isTranscoding && !sseConnections.current[ep.id]) {
  ...
  return (
    <div className="min-h-screen   text-white p-2 md:p-6 mt-16 md:mt-0">
      ...
      <div className="flex flex-row items-center w-full justify-end p-2">
        <button
          className="p-2 rounded-md bg-neutral-800 flex flex-row items-center"
          onClick={() => {
            fetchSeries()
          }}
        >
